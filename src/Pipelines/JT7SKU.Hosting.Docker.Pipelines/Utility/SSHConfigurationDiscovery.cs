...
using JT7SKU.Hosting.Docker.Pipelines.Models;
        ...
        private static void ParseKnownHostsFile(string filePath, List<string> knownHosts)
        {
            try
            {
                ...
                foreach (var line in lines)
                {
                    ...
                    {
                        var puhdasHost = host.Trim();
                        if (string.IsNullOrEmpty(puhdasHost)) continue;

                        // Remove port specification (e.g., [hostname]:port -> hostname)
                        if (puhdasHost.StartsWith("[") && puhdasHost.Contains("]:"))
                        {
                            var endBracket = puhdasHost.IndexOf("]:");
                            if (endBracket > 1)
                            {
                                puhdasHost = puhdasHost[1..endBracket];
                            }
                        }

                        // Skip if still contains problematic characters (likely hashed or malformed)
                        if (puhdasHost.Contains("|") || puhdasHost.Contains("=")) continue;

                        hostSet.Add(puhdasHost);
                    }
                }
